...
const REGIONS = {
  north: { name: '北日本方面', short: 'NORTH', console: '北部防衛回線', description: '北海道・東北', color: '#86c5d9' },
  east: { name: '東日本方面', short: 'EAST', console: '東部防衛回線', description: '関東', color: '#d8bd66' },
  central: { name: '中日本方面', short: 'CENTRAL', console: '中部防衛回線', description: '甲信越・北陸・東海', color: '#a7c984' },
  west: { name: '西日本方面', short: 'WEST', console: '西部防衛回線', description: '近畿・中国', color: '#cf9ab7' },
  south: { name: '南日本方面', short: 'SOUTH', console: '南部防衛回線', description: '四国・九州・沖縄', color: '#df8f61' },
  national: { name: '全国防衛作戦', short: 'NATIONAL', console: '統合防衛司令部', description: '全国47都道府県', color: '#f1df97' },
};
...
function renderStory() {
  ...
  app.innerHTML = `
    <main class="story-screen screen" style="--story-image: url('${slide.image}')">
      ...
      <footer class="story-footer"><button class="text-button" data-action="skip-story">スキップ</button><button class="primary-button" data-action="next-story">${isLast ? '防衛回線を選ぶ' : '次へ'} <span>→</span></button></footer>
    </main>`;
}
...
function renderModes() {
  ...
  const cards = sectors().map((regionId) => {
    ...
    return `<button class="sector-card ${done ? 'is-cleared' : ''}" data-action="briefing" data-region="${regionId}">
      <span class="sector-index">${region.short}</span><span class="sector-status">${done ? '回線復旧済' : '調整待機'}</span>
  ...
  app.innerHTML = `
    <main class="mode-screen screen">
      <header class="page-header"><div><p class="console-label">DEFENCE NETWORK / ${countProgress()} OF 5 LINKS</p><h2>防衛回線を選ぶ</h2><p>方面回線を復旧し、全国防衛作戦を解放せよ。</p></div><button class="text-button" data-action="intro">終了</button></header>
      <section class="sector-list">${cards}</section>
      <section class="national-card ${nationalAvailable ? 'is-ready' : 'is-locked'}">
        <div><p class="console-label">FINAL OPERATION</p><strong>全国防衛作戦</strong><small>全国47都道府県 / 統合防衛司令部</small></div>
        <button class="primary-button" data-action="briefing" data-region="national" ${nationalAvailable ? '' : 'disabled'}>${nationalAvailable ? '最終防衛を開始' : `あと ${5 - countProgress()} 回線`}</button>
      </section>
      <p class="mode-note">各方面回線でC評価以上を取ると回線が復旧します。最終作戦では全国47都道府県を連続で防衛します。</p>
    </main>`;
}
...
function renderBriefing(regionId) {
  ...
  app.innerHTML = `
    <main class="briefing-screen screen">
      <header class="page-header"><div><p class="console-label">${national ? 'FINAL / NATIONAL DEFENCE' : `SECTOR / ${region.short}`}</p><h2>${region.name}</h2></div><button class="text-button" data-action="modes">回線一覧へ</button></header>
      ...
      <aside class="briefing-callout">${national ? '全国防衛作戦です。結果は今回の作戦記録として保存されます。' : '回線復旧条件：正解率50%以上、防衛成功60%以上、さらに人口または面積を70%以上防衛。'}</aside>
      <button class="primary-button wide-button" data-action="begin" data-region="${regionId}">${national ? '全国防衛を開始する' : '方面防衛を開始する'} <span>→</span></button>
    </main>`;
}
...
function playImpact(target, correct, kind) {
  ...
  if (!correct) {
    ...
    const fly = (now) => {
      ...
      else { missile.remove(); trail.classed('is-complete', true); showExplosion(overlay, targetPoint); }
    };
...
function renderResults() {
  ...
  const label = national ? '全国防衛記録' : `${REGIONS[run.regionId].name}・回線記録`;
  ...
  app.innerHTML = `
    <main class="result-screen screen">
      <section class="result-hero"><p class="console-label">${national ? 'FINAL DEFENCE LOG' : 'SECTOR EVALUATION'}</p><h2>${label}</h2><div class="grade grade-${result.grade}">${result.grade}<small>GRADE</small></div><p>${national ? 'あなたが守れた日本の記録です。' : result.cleared ? '防衛回線を復旧しました。' : '回線復旧の基準に届きませんでした。'}</p></section>
      ...
      <section class="result-callout">${national ? '残った地図が、あなたの守った日本です。' : result.cleared ? `演算キーを獲得。全国防衛作戦まで、残り ${5 - countProgress()} 回線。` : '正解率50%、防衛成功60%、人口または面積70%を目標に再挑戦してください。'}</section>
      ...
      <footer class="result-actions"><button class="primary-button" data-action="retry">同じ回線を再挑戦 <span>↻</span></button><button class="outline-button" data-action="modes">防衛回線一覧</button></footer>
    </main>`;
