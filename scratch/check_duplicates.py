    ...
    if repo_name:
        ...
        if repo_name_lower in seen_repo_names:
            ...
            duplicates.append({
                ...
                'repoName': repo_name,
                'old_repoName': articles[orig_idx].get('repoName')
            })
            is_duplicate = True
        else:
    ...
    if not is_duplicate and repo_url:
        ...
        if repo_url_lower in seen_repo_urls:
            ...
            duplicates.append({
                ...
                'repoUrl': repo_url,
                'old_repoUrl': articles[orig_idx].get('repoUrl')
            })
            is_duplicate = True
        else:
...
if duplicates:
    ...
    for d in duplicates:
        print(f"NEW Index {d['new_idx']} ({d['article'].get('publishedAt')}) duplicate of OLD Index {d['old_idx']} ({d['old_article'].get('publishedAt')})")
        print(f"  Reason: {d['reason']}")
        print(f"  New: {d['article'].get('title')} | {d.get('repoName')} | {d.get('repoUrl')}")
        print(f"  Old: {d['old_article'].get('title')} | {d.get('old_repoName')} | {d.get('old_repoUrl')}")
else:
